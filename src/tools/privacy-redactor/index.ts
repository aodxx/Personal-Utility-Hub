...
const patterns = [
  { key: 'secret', label: 'Secret / credential', regex: /\b(?:token|secret|api[_-]?key|access[_-]?key|password|client[_-]?secret)\s*[:=]\s*(?:Bearer\s+)?[A-Za-z0-9._~+/=-]{8,}/gi },
  { key: 'secret', label: 'Secret / credential', regex: /\bBearer\s+[A-Za-z0-9._~+/=-]{20,}/gi },
  { key: 'secret', label: 'Secret / credential', regex: /\b(?:sk|pk|rk)-[A-Za-z0-9_-]{10,}\b/gi },
  { key: 'secret', label: 'Secret / credential', regex: /\bAKIA[0-9A-Z]{16}\b/g },
  ...
  { key: 'token', label: 'API/token-like value', regex: /\b(?:token|secret|key)[_-]?[A-Za-z0-9_-]{12,}\b/gi },
] as const;

export function redactText(value: string): { output: string; counts: Record<string, number>; total: number } {
  ...
  for (const pattern of patterns) {
    ...
    counts[pattern.label] = (counts[pattern.label] ?? 0) + count;
  }
...
const tool: ToolModule = {
  ...
  mount(container) {
    ...
    panel.addEventListener('click', async (event) => { const action = (event.target as HTMLElement).closest<HTMLElement>('[data-redactor-action]')?.dataset.redactorAction; if (!action) return; const status = panel!.querySelector<HTMLOutputElement>('#redactor-status')!; try { if (action === 'scan') { update(panel!); const text = panel!.querySelector<HTMLTextAreaElement>('#redactor-result')!.value; setDownload(panel!, text); const total = Object.values(redactText(panel!.querySelector<HTMLTextAreaElement>('#redactor-input')!.value).counts).reduce((sum, count) => sum + count, 0); setToolStatus(status, total ? `ปิดบังข้อมูลแล้ว ${total} รายการ / Redacted ${total} item(s)` : 'ไม่พบรูปแบบที่รองรับ กรุณาตรวจทานด้วยตนเอง / No supported patterns found; review manually', total ? 'success' : 'neutral'); } if (action === 'copy') { await copyText(panel!.querySelector<HTMLTextAreaElement>('#redactor-result')!.value); setToolStatus(status, 'คัดลอกผลลัพธ์แล้ว / Copied', 'success'); } if (action === 'download' && outputUrl) downloadUrl(outputUrl, `redacted-${sourceName.replace(/\.[^.]+$/, '')}.txt`); if (action === 'clear') { input.value = ''; panel!.querySelector<HTMLTextAreaElement>('#redactor-result')!.value = ''; panel!.querySelector<HTMLElement>('#redactor-report')!.textContent = 'ยังไม่มีการตรวจข้อมูล / No scan yet'; } } catch (error) { setToolStatus(status, getErrorMessage(error), 'error'); } });
