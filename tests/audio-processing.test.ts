...
describe('Audio processing', () => {
  ...
  it('runs the five production audio operations with real output metrics', () => {
    ...
    const mp3 = processAudio(source, { kind: 'merge', segments: [source], gap: 0, crossfade: 0, format: 'mp3' });
    ...
    expect(mp3.outputFormat).toBe('mp3');
    expect(mp3.bytes.length).toBeGreaterThan(100);
    ...
    expect(finisher.rmsDb).toBeLessThan(0);
    expect(finisher.truePeak).toBeGreaterThan(0);
    expect(finisher.inputRmsDb).toBeLessThan(0);
