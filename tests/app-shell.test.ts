...
describe('AppShell integration', () => {
  ...
  it('renders the Hub and filters tools by Thai search and category', async () => {
    ...
    expect(root.querySelectorAll('#tool-grid .tool-card')).toHaveLength(32);
    expect(root.querySelectorAll('.category-tab .asset-icon')).toHaveLength(13);
    expect(root.querySelectorAll('#tool-grid .tool-card .asset-icon')).toHaveLength(32);
  ...
  it('persists favorites, records recent tools and clears history', async () => {
    const root = startApp();
    await vi.waitFor(() => expect(root.querySelectorAll('#tool-grid .tool-card')).toHaveLength(32));
