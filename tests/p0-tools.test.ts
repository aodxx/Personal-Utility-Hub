...
import { compareDigest, hashText } from '../src/core/hash';
import { inspectJwt } from '../src/core/jwt';
...
describe('P0 tool primitives', () => {
  ...
  it('hashes UTF-8 text and compares normalized digests', async () => {
    ...
    expect(compareDigest(result.value, '2C:F2 4D BA 5F B0 A3 0E 26 E8 3B 2A C5 B9 E2 9E 1B 16 1E 5C 1F A7 42 5E 73 04 33 62 93 8B 98 24')).toBe('match');
    expect(compareDigest(result.value, 'not-hex')).toBe('invalid-expected');
  });
